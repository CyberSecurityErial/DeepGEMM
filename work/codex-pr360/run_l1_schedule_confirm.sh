...
SITE=${SITE:-/tmp/codex-pr360-site-current-20260624-140358}
RUNS=${RUNS:-5}
NUM_TESTS=${NUM_TESTS:-7}
RUN_ROOT=${RUN_ROOT:-/tmp/codex-pr360-l1-confirm-$(date +%Y%m%d-%H%M%S)}

if [[ ! -d "$SITE/deep_gemm" ]]; then
  echo "Missing clean DeepGEMM install: $SITE" >&2
  echo "Rebuild the clean /tmp install before running this script." >&2
  exit 1
fi
...
run_case() {
  ...
  (
    ...
    unset DG_SM90_MOE_L1_NMAJOR DG_SM90_MOE_EXPERT_LOCAL DG_SM90_MOE_NMAJOR
    eval "$extra_env"
...
for ((i = 1; i <= RUNS; ++i)); do
  ...
  run_case MiMoPro 6144 2048 8 l1n "export DG_SM90_MOE_L1_NMAJOR=1" "$i"
  run_case V4Pro 7168 3072 6 base "" "$i"
  run_case V4Pro 7168 3072 6 both "export DG_SM90_MOE_L1_NMAJOR=1; export DG_SM90_MOE_EXPERT_LOCAL=1" "$i"
done
...
for log in sorted(root.glob("*.log")):
    m = re.match(r"(MiMoPro|V4Pro)_(base|l1n|both)_run(\d+)\.log", log.name)
    if not m:
        continue
    text = log.read_text(errors="ignore")
...
for model, cand in (("MiMoPro", "l1n"), ("V4Pro", "both")):
    base = vals.get((model, "base"), [])
    opt = vals.get((model, cand), [])
    if len(base) == len(opt) and base:
        print(f"SPEEDUP {model} {cand} {(st.mean(base) / st.mean(opt) - 1) * 100:.2f}%")
    else:
        print(f"SPEEDUP {model} {cand} incomplete base_n={len(base)} opt_n={len(opt)}")
