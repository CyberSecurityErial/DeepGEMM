...
#include "../heuristics/sm90_fp8_mega_moe.hpp"
...
public:
    struct Args {
        ...
        // Tensormaps and weight SF pointers
...
static void sm90_fp8_mega_moe(
    ...
    // Heuristics
    const auto config = get_sm90_fp8_mega_moe_config(
        num_ranks, num_experts, num_experts_per_rank,
        num_max_tokens_per_rank, num_tokens, num_topk,
        hidden, intermediate_hidden, num_padded_sf_pool_tokens);
    const int epilogue_registers = config.num_epilogue_threads == 512 ? 112 : 0;
    ...
    const bool split_n_warpgroups =
        config.block_m == 64 and num_epilogue_warpgroups > 1 and
        config.block_n % num_epilogue_warpgroups == 0 and
        (config.block_n / num_epilogue_warpgroups == 64 or
         config.block_n / num_epilogue_warpgroups == 128);
    const bool split_mn_warpgroups =
        config.block_m == 128 and config.block_n == 256 and num_epilogue_warpgroups == 4;
