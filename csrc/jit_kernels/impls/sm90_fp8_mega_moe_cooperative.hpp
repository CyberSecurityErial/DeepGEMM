...
// ----------------------------------------------------------------------------
// SM90 counterpart of `SM100FP8FP4MegaMoERuntime`, driving the cooperative
// kernel in `deep_gemm/impls/sm90_fp8_mega_moe_cooperative.cuh` (BLOCK_M=128,
// two math warpgroups cooperatively M-split the same tile and share one B-tile
// load, halving weight HBM traffic). Best for large M.
//
...
// cooperative_v10: optional M-local L1->L2 scheduler behind DG_SM90_MOE_MLOCAL.
...
static void __instantiate_kernel() {{
    ...
    args.config.mlocal_schedule ? "true" : "false",
...
static void sm90_fp8_mega_moe_cooperative(
    ...
    // cooperative: each math warpgroup owns its OWN 64-row band (WG_BLOCK_M =
    // block_m / 2) at `m_idx + row_block_offset` and stores it via a SEPARATE
    // TMA op, so the descriptor outer-box dim must be `block_m / 2`
    // (=WG_BLOCK_M), NOT `block_m` — otherwise the per-WG store would address
    // rows outside its half-tile.
    const auto tensor_map_l1_output = make_tma_2d_desc(l2_acts,
                                                       intermediate_hidden, config.num_max_pool_tokens,
                                                       config.block_n / 2, config.block_m / 2,
