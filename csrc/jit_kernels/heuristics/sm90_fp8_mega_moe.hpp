...
struct SM90FP8MegaMoEConfig {
    ...
    friend std::ostream& operator << (std::ostream& os, const SM90FP8MegaMoEConfig& config) {
        os << "SM90FP8MegaMoEConfig("
...
static std::tuple<int, int> get_block_config_for_sm90_fp8_mega_moe(
...
static int get_num_experts_per_wave_for_sm90_fp8_mega_moe(
...
static std::pair<int, int> get_pipeline_config_for_sm90_fp8_mega_moe(
    ...
    const int smem_per_stage = block_m * block_k + block_n * block_k + smem_sfa_per_stage;
...
static SM90FP8MegaMoEConfig get_sm90_fp8_mega_moe_config(
    const int& num_ranks, const int& num_experts, const int& num_experts_per_rank,
    const int& num_max_tokens_per_rank, const int& num_tokens, const int& num_topk,
    const int& hidden, const int& intermediate_hidden,
    const int& num_padded_sf_pool_tokens) {
    const auto [block_m, num_epilogue_threads] = get_block_config_for_sm90_fp8_mega_moe(
    ...
    const int num_experts_per_wave = get_num_experts_per_wave_for_sm90_fp8_mega_moe(
        num_experts_per_rank, num_tokens, num_topk,
        intermediate_hidden, block_m, block_n, num_sms);
    ...
    const auto [num_stages, smem_size] = get_pipeline_config_for_sm90_fp8_mega_moe(
        SM90ArchSpec::smem_capacity,
        num_experts, hidden,
        block_m, block_n, block_k,
        num_dispatch_threads / 32, num_epilogue_threads / 32);

    const auto config = SM90FP8MegaMoEConfig {
        block_m, block_n, block_k,
        cluster_size,
    ...
    if (get_env<int>("DG_JIT_DEBUG") or get_env<int>("DG_PRINT_CONFIGS")) {
        const auto key = fmt::format(
            "SM90FP8MegaMoEConfig(num_ranks={}, num_experts={}, hidden={}, intermediate_hidden={}, num_max_tokens_per_rank={}, num_tokens={}, num_topk={})",
