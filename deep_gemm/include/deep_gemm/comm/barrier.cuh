...
CUTLASS_DEVICE void nvlink_barrier(const layout::Workspace& workspace,
    ...
    if (sm_idx == 0) {
        ...
        !(defined(DG_NVLINK_BARRIER_VERBOSE_TIMEOUT) && DG_NVLINK_BARRIER_VERBOSE_TIMEOUT)
                    // Device printf can make an already wedged Hopper communication
                    // kernel substantially harder to terminate. Keep the detailed
                    // diagnostic available behind an explicit compile-time switch.
