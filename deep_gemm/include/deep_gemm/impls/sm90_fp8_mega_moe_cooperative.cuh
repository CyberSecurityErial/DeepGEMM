...
// ============================================================================
// SM90 (Hopper) FP8 MegaMoE — cooperative kernel
// ----------------------------------------------------------------------------
// BLOCK_M=128: the two math warpgroups cooperatively M-split a single tile
// (each owns a 64-row band) and share one B-tile (weight) load, halving weight
// HBM traffic. A 256-thread cross-warpgroup barrier closes the L2 epilogue so a
// warpgroup cannot overwrite the shared CD SMEM while the other still scatters
// it (required when an SM runs more than one tile). Tuned for large M.
//
...
#pragma unroll
        ...
        // COOP M-split: the two math WGs cooperatively process the SAME tile, each
        // owning HALF the BLOCK_M rows. WG g owns rows [g*WG_BLOCK_M, (g+1)*WG_BLOCK_M).
        // One m64 WGMMA per WG per k-block; the shared B tile is loaded ONCE and read
        // by both WGs (this halves the weight HBM traffic vs pingpong's per-tile B).
        constexpr uint32_t WG_BLOCK_M = BLOCK_M / kNumEpilogueWarpgroups;        // 64
        DG_STATIC_ASSERT(WG_BLOCK_M == L1WGMMA::M, "Cooperative: WG_BLOCK_M must equal one WGMMA M (64)");
        DG_STATIC_ASSERT(kNumEpilogueWarpgroups == 2, "Cooperative requires exactly 2 math warpgroups");
        // This WG's row band within the BLOCK_M-row tile (WG0: 0, WG1: WG_BLOCK_M).
        const uint32_t row_block_offset = epilogue_wg_idx * WG_BLOCK_M;
        // A/SFA SMEM offsets for this WG's rows (B is shared → no N offset).
        const uint32_t a_row_off  = row_block_offset * BLOCK_K; // bytes-in-elems into smem_a
        const uint32_t sfa_row_off = row_block_offset;          // into smem_sfa (per-row floats)
        ...
        while (true) {
            ...
            // own WG_BLOCK_M row band of this tile via row_block_offset.
...
#pragma unroll
                    for (uint32_t k = 0; k < BLOCK_K / WGMMA::K; ++k) {
                        // COOP: this WG reads its own 64 A-rows (a_row_off); B shared.
...
#pragma unroll
                    for (uint32_t i = 0; i < kAccumPerThread / 4; ++i) {
                        ...
                        final_accum[i * 4 + 0] += s0 * accum[i * 4 + 0];
                        final_accum[i * 4 + 1] += s0 * accum[i * 4 + 1];
                        final_accum[i * 4 + 2] += s1 * accum[i * 4 + 2];
                        final_accum[i * 4 + 3] += s1 * accum[i * 4 + 3];
                    }
                } else {
...
#pragma unroll
                    for (uint32_t i = 0; i < kAccumPerThread / 4; ++i) {
                        ...
                        final_accum[i * 4 + 1] += s0_lo * accum[i * 4 + 1];
                        final_accum[i * 4 + 2] += s1_lo * accum[i * 4 + 2];
                        final_accum[i * 4 + 3] += s1_lo * accum[i * 4 + 3];
                    }
...
#pragma unroll
                    for (uint32_t k = 0; k < (BLOCK_K / 2) / WGMMA::K; ++k) {
                        ...
                        auto desc_a          = mma::sm90::make_smem_desc(
                            smem_a[stage_idx] + a_row_off + k_off, 1);
                        auto desc_b = mma::sm90::make_smem_desc(
                    ...
                    // Read L2 hi-half act SF while second-half WGMMA is executing
                    scale_a_0_hi = ptx::ld_shared(smem_sfa[stage_idx] + 1 * BLOCK_M + sfa_row_off + r_0);
                    scale_a_1_hi = ptx::ld_shared(smem_sfa[stage_idx] + 1 * BLOCK_M + sfa_row_off + r_1);
...
#pragma unroll
                    ...
                    // act-SF × weight-SF once (2 muls) → inner loop is pure FMA.
                    const float s0_hi = scale_a_0_hi * cur_l2_sf;
                    const float s1_hi = scale_a_1_hi * cur_l2_sf;
#pragma unroll
                    for (uint32_t i = 0; i < kAccumPerThread / 4; ++i) {
                        final_accum[i * 4 + 0] += s0_hi * accum[i * 4 + 0];
                        final_accum[i * 4 + 1] += s0_hi * accum[i * 4 + 1];
                        final_accum[i * 4 + 2] += s1_hi * accum[i * 4 + 2];
...
#pragma unroll
                ...
                // COOP: this WG's tokens are pool_block_idx*BLOCK_M + row_block_offset + r_*.
                if (col_idx == 0) {
                    auto sf_base_ptr = l2_sf_buffer.get_base_ptr<float>();
                    const uint32_t token_r0                                   = pool_block_idx * BLOCK_M + row_block_offset + r_0;
                    const uint32_t token_r1                                   = pool_block_idx * BLOCK_M + row_block_offset + r_1;
                ...
                // Sync this WG before its TMA store (per-WG barrier).
                ptx::sync_aligned(128, kEpilogueWGBarrierStartIdx + epilogue_wg_idx);

                // Each WG TMA-stores its OWN WG_BLOCK_M-row band at m_idx + row_block_offset.
...
#pragma unroll
                for (uint32_t j = 0; j < kNumRowsPerWarp; ++j) {
                    // COOP: row within this WG's band; global tile row adds row_block_offset.
                    const uint32_t row_in_wg     = warp_idx_in_wg * 16 + j * 2 + row_in_warp_block;
                    const uint32_t tile_row       = row_block_offset + row_in_wg;
                    if (tile_row >= valid_m)
                        break;
        ...
        for (uint32_t token_idx = sm_idx * kNumEpilogueWarps + epilogue_warp_idx;
            ...
            const int stored_topk_slot_idx = lane_idx < kNumTopk ? static_cast<int>(__ldg(input_topk_idx_buffer.get_base_ptr<int64_t>() + token_idx * kNumTopk + lane_idx)) : -1;
