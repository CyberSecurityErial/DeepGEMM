...
struct MegaMoEScheduler {
    ...
    CUTLASS_DEVICE uint32_t get_valid_m() const {
        // Guard the trailing m-block: under N-major scheduling the (m,n) visit
        // order changes, so a partial/empty trailing m-block can be reached on a
        // path where `m_block_idx * BLOCK_M >= current_num_tokens` (underflow).
    ...
    CUTLASS_DEVICE bool fetch_next_l2_block() {
        ...
        while (current_local_expert_idx < wave_end_expert_idx) {
            ...
            if (block_idx < num_m_blocks * kNumL2BlockNs) {
                // N-major: n outer, m inner -> consecutive blocks share n_block
                // (same weight column), so the L2 weight tile stays resident in
                // L2 while we sweep all m (activation) blocks. M-major: n inner.
        ...
        while (true) {
            ...
            if (next_phase == BlockPhase::Linear1) {
                ...
                if (fetch_next_l2_block()) {
                    // Found a new L2 block (m_block_idx / n_block_idx already set
                    // inside fetch_next_l2_block per the N-major/M-major order)
                    // Jump to next block
                    block_idx += kNumSMs;
                    return {BlockPhase::Linear2, current_local_expert_idx, m_block_idx, n_block_idx};
                } else {
